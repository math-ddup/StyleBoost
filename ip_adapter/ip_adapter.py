...
class ImageProjModel(torch.nn.Module):
    """Projection Model
    通过投影和归一化将图像embedding投影到新的空间，使图像和文本信息可以在交叉注意力机制中共同作用的embedding
    """
...
class IPAdapter:
    ...
    def set_scale_1(self, scale_1):
        for attn_processor in self.pipe.unet.attn_processors.values():
            if isinstance(attn_processor, IPAttnProcessor_cross_modal):
                attn_processor.scale_1 = scale_1
...
class IPAdapterXL(IPAdapter):
    ...
    def get_image_embeds(self, pil_image=None, clip_image_embeds=None, content_prompt_embeds=None,
        ...
        # SVD + 奇异值增强
        B, D = clip_image_embeds.shape
        clip_image_embeds_2d = clip_image_embeds.view(B, D).to(dtype=torch.float32)
        print(f'clip_image_embeds_2d.shape={clip_image_embeds_2d.shape}')

        # 奇异值分解
        U, S, Vh = torch.linalg.svd(clip_image_embeds_2d, full_matrices=False)
        print(f'S={S}')

        S_enhanced = beta*torch.exp(-alpha*S)*S
        # S_enhanced = S/1
        print(f'S_enhanced={S_enhanced}')

        # 重构增强后的嵌入
        A_enhanced = torch.diag(S_enhanced)
        clip_image_embeds_enhanced = U @ A_enhanced @ Vh
        clip_image_embeds = clip_image_embeds_enhanced.to(dtype=torch.float16)
